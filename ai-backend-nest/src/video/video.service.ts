...
export class VideoService {
  ...
  async generateVideoPrompts(file: Express.Multer.File, productName: string) {
    ...
    // B. 上传到 OSS
    await this.ossClient.put(key, compressedBuffer, {
    ...
    const visionRes = await this.visionModel
    ...
    const creativeRes = await this.creativeModel
      .pipe(parser)
      .invoke([new HumanMessage(promptSystem)]);

    this.logger.log(
      `[Video] 3/3 脚本生成完毕，共 ${creativeRes.prompts.length} 组`,
    );
  ...
  async submitKlingTask(imageUrl: string, prompt: string) {
    ...
    const apiBaseUrl =
      this.configService.get('KLING_API_URL') ||
      'https://api-beijing.klingai.com/v1';
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        ...
        this.logger.error(
          `[Kling] HTTP错误: ${status} - ${JSON.stringify(data)}`,
        );
        ...
        throw new HttpException(
          String(data.message || `服务商请求失败 (${status})`),
  ...
  async getKlingStatus(taskId: string) {
    ...
    try {
      const response = await axios.get(
        `${apiBaseUrl}/videos/image2video/${taskId}`,
        { headers },
      );
      ...
      if (normalizedStatus === 'SUCCEEDED') {
        videoUrl =
          taskData.task_result?.videos?.[0]?.url || taskData.output?.url;
      }
      ...
      return { status: 'RUNNING', video_url: '', message: error.message }; // 报错也继续轮询
    }
  }
