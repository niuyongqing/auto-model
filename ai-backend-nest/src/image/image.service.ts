...
export class ImageService {
  ...
  constructor(private configService: ConfigService) {
    ...
    if (!apiKey) {
      this.logger.error(
        '❌ 严重错误: 未能在环境变量中找到 DASHSCOPE_API_KEY！',
      );
      this.logger.error('   -> 请检查 .env 文件是否存在且位于项目根目录。');
      this.logger.error(
        '   -> 请检查 ConfigModule 是否在 AppModule 中正确配置。',
      );
    } else {
      // 安全地打印 Key 的前几位，确认读取成功
      this.logger.log(
        `✅ LangChain 初始化成功，使用 API Key: ${apiKey.substring(0, 8)}...`,
      );
    }
  ...
  async generateImages(file: Express.Multer.File, productName: string) {
    try {
      ...
      const originalUrl = await this.uploadBufferToOSS(
        file.buffer,
        originalKey,
        mimeType,
      );
      ...
      for (const strategy of strategies) {
        try {
          const imageUrl = await this.callWanxApi(
            transparentOssUrl,
          ...
          this.logger.error(
            `风格 [${strategy.title}] 生成失败: ${err.message}`,
          );
          continue;
        }
      }
      ...
      "strategies": [
        {
          "style": "风格名称 (例如: 极简风)",
          "title": "营销标题 (例如: 夏季必备)",
          "image_prompt": "生图提示词 (重要: 只描述背景环境、光影、氛围，不要描述商品本身)"
        }
      ]
    }
  ...
  async processSegmentation(inputBuffer: Buffer): Promise<string> {
    ...
    const tempInputPath = path.resolve(
      process.cwd(),
      `temp_in_${timestamp}.png`,
    );
    const tempOutputPath = path.resolve(
      process.cwd(),
      `temp_out_${timestamp}.png`,
    );
    ...
    try {
      ...
      const { stderr } = await execAsync(
        `node "${workerScript}" "${tempInputPath}" "${tempOutputPath}"`,
      );
      ...
      return await this.uploadBufferToOSS(
        finalRgbaBuffer,
        transparentKey,
        'image/png',
      );
